...
def load_model(model_path, config, device):
    ...
    if ckpt_h != cfg_h or ckpt_w != cfg_w:
        ...
        print(f"\nTo fix this, either:")
        print(
            f"  1. Update config/config.yaml to use width: {ckpt_w}, height: {ckpt_h}"
        )
...
def predict_future_frames(
    ...
    with torch.no_grad():
        ...
        input_sequence = all_frames_tensor[
            -input_frames_count:
        ]  # [input_frames_count, C, H, W]
        input_batch = input_sequence.unsqueeze(0).to(
            device
        )  # [1, input_frames_count, C, H, W]

        # Predict future frames (with residual for debugging)
        output, residual = model(
            input_batch, return_residual=True
        )  # [1, output_frames_count, C, H, W]
        ...
        if save_debug and output_dir:
            ...
            for i in range(output_frames_count):
                ...
                residual_abs_amplified = (residual_abs * 10).clamp(
                    0, 1
                )  # Amplify by 10x
                residual_abs_np = tensor_to_frame(residual_abs_amplified)

                # Save residual images
                Image.fromarray(residual_vis_np).save(
                    debug_dir / f"residual_{i:06d}.png"
                )
                Image.fromarray(residual_abs_np).save(
                    debug_dir / f"residual_abs_{i:06d}.png"
                )

                # Print per-frame residual stats
                print(
                    f"Frame {i}: residual mean={residual_frame.mean().item():.6f}, "
...
def main():
    ...
    parser.add_argument(
        "--output_dir", type=str, help="Output directory for predictions"
    )
    parser.add_argument(
        "--model_path", type=str, help="Path to trained model checkpoint"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Save intermediate residual images for debugging",
    )
    ...
    if not Path(video_path).exists():
        ...
        print(
            "Please specify a valid video path in config/config.yaml (paths.input_video)"
        )
        return
    ...
    if len(all_frames_numpy) < config["model"]["input_frames"] + 1:
        print(
            f"Not enough frames in video. Need at least {config['model']['input_frames'] + 1}, got {len(all_frames_numpy)}"
        )
        return
    ...
    print(
        f"Using last {config['model']['input_frames']} frames to predict {config['model']['output_frames']} future frame(s)..."
    )
